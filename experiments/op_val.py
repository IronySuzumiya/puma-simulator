...
zero_offset = 2** (xbar_extraBits-1)     # ?? This BIAS has any relation to the zero_offset mentioned in line 116?
...
def wt_bitSlice (wt):
    ...
    for i in range (num_bits//xbar_bits):
        ...
        temp_int = bin2int(temp,xbar_extraBits) + zero_offset # ??? Why +1?
...
def outer_prod2 (wt_sliced, in1, in2, sub=0):
    # to fixed point
    if in1 < 0:
        in1 = abs(in1) + (1 << num_bits)
    if in2 < 0:
        in2 = abs(in2) + (1 << num_bits)
    for i in range (num_bits):
        lsb1 = in1 % 2 # get LSB
        if lsb1 == 1:
            op2 = in2 << i
