...
class noc (object):
    ...
    def check_inter (self, src_tileId, dest_tileId):
        inter_flag = 1
        src_nodeId = (src_tileId-1) / cfg.num_tile_compute # integer division
        dest_nodeId = (dest_tileId-1) / cfg.num_tile_compute

        # Note: Input (Output) dummy tile are assumed to communicate (communicated) by intra-node NoC
        if (src_tileId == 0 or dest_nodeId == (cfg.num_node * cfg.num_tile_compute + 2)\
                or src_nodeId == dest_nodeId):
            inter_flag = 0
        return inter_flag
    ...
    # Propagate just adds to inter_noc access (router power) or ht access based on src & dest address
    ...
    def stop_noc (self, cycle):
        self.num_access_intra += (cycle - self.start_cycle+1)
