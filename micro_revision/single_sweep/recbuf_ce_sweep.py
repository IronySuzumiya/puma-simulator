...
# single sweep parameter - receive buffer virtualization (16 v_tiles), no virtualization (138 v_tiles)
...
def get_tile_area (xbar_size, num_xbar, num_vfu, num_core, recbuf_size):
    core_area = 0.0
    ...
    # Less send and receives between tiles
    tile_area += receive_buffer_area/ (math.sqrt((xbar_size/128.0)*(num_xbar/16.0))) * (recbuf_size**2)
    tile_area += tcu_area
        ...
        for num_xbar in num_xbar_list:
            for num_core in num_core_list:
                for recbuf_size in recbuf_size_list:
                    tile_area = get_tile_area (xbar_size, num_xbar, num_vfu, num_core, recbuf_size)
