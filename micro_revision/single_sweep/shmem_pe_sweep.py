...
# single sweep parameter - shared memory size
shmem_size_list = [0.25, 1, 4, 16]
...
def get_tile_power (xbar_size, num_xbar, num_vfu, num_core, shmem_size):
    core_power = 0.0
    ...
    tile_power += math.sqrt(num_core/8.0) * edram_pow * shmem_size
    ...
    for num_vfu in num_vfu_list:
        for num_xbar in num_xbar_list:
            for num_core in num_core_list:
                for shmem_size in shmem_size_list:
                    #tile_energy = get_tile_energy (xbar_size, num_xbar, num_vfu, num_core)
                    tile_power = get_tile_power (xbar_size, num_xbar, num_vfu, num_core, shmem_size)
