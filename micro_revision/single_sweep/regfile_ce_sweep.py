...
def get_tile_area (xbar_size, num_xbar, num_vfu, num_core, regfile_size, embrom):
    core_area = 0.0
    ...
    core_area += math.sqrt((xbar_size/128.0) * (num_xbar/16.0)) * dataMem128_2_area * regfile_size * rom_scaling
            ...
            for num_core in num_core_list:
                for regfile_size in regfile_size_list:
                    for embrom in embrom_list:
                        tile_area = get_tile_area (xbar_size, num_xbar, num_vfu, num_core, regfile_size, embrom)
                        ...
                        ce = 2*num_comp_total / tile_area / time / (10**9) #in GigaOps
                        name = 'regfile_sz' + str(regfile_size) + 'embrom' + str(embrom)
                        #ce_dict[name] = ce
                        ce_list.append(round(ce,2))
