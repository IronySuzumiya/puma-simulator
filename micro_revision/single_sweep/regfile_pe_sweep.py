...
xbar_size_list = [128]
num_xbar_list = [32] # [xbar_size (DACs,SnH), 1 ADC, 1 SnA per xbar]
num_vfu_list = [1]
num_core_list = [16]

# single sweep parameter - register file size
regfile_size_list = [0.25, 1, 4, 16]
embrom_list = [0, 1] #0 - regular LUT, 1-Emb ROM
...
def get_tile_power (xbar_size, num_xbar, num_vfu, num_core, regfile_size, embrom):
    core_power = 0.0
    ...
    rom_scaling = 2 if (embrom == 0) else 1 #larger memory requirement to store lut if not emb-rom ram
    core_power += math.sqrt((xbar_size/128.0) * (num_xbar/16.0)) * dataMem128_2_pow * regfile_size * rom_scaling
        ...
        for num_xbar in num_xbar_list:
            for num_core in num_core_list:
                for regfile_size in regfile_size_list:
                    for embrom in embrom_list:
                        #tile_energy = get_tile_energy (xbar_size, num_xbar, num_vfu, num_core)
                        tile_power = get_tile_power (xbar_size, num_xbar, num_vfu, num_core, regfile_size, embrom)
                        ...
                        # Multiplying by 2 to account for add and mul separately
                        pe = 2*num_comp_total/ tile_power / (10**3) #inGops/Watt
                        name = 'regfile_sz' + str(regfile_size) + 'embrom' + str(embrom)
                        #ce_dict[name] = ce
