...
# single sweep parameter - receive buffer virtualization (16 v_tiles), no virtualization (num_tiles on node)
recbuf_size_list = [0.25, 1, 4, 16]
...
def get_tile_power (xbar_size, num_xbar, num_vfu, num_core, recbuf_size):
    core_power = 0.0
    ...
    # Less send and receives between tiles
    tile_power += receive_buffer_pow/ (math.sqrt((xbar_size/128.0)*(num_xbar/16.0))) * (recbuf_size**2)
    tile_power += tcu_pow
    ...
    for num_vfu in num_vfu_list:
        for num_xbar in num_xbar_list:
            for num_core in num_core_list:
                for recbuf_size in recbuf_size_list:
                    #tile_energy = get_tile_energy (xbar_size, num_xbar, num_vfu, num_core)
                    tile_power = get_tile_power (xbar_size, num_xbar, num_vfu, num_core, recbuf_size)
                    ...
                    name = 'recbuf_sz' + str(recbuf_size)
                    #ce_dict[name] = ce
                    if (recbuf_size < 1.0):
                        pe_list.append('nan')
                    else:
