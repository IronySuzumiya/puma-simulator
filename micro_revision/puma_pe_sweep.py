...
## single sweeps (sweet spot S128-X16-V4-C4, 772.13)
xbar_size_list = [128]
num_mvmu_list = [16]
num_vfu_list = [4]
num_core_list = [4]
...
def get_tile_power (xbar_size, num_xbar, num_vfu, num_core, regfile_size):
    core_power = 0.0
    # assumption xbar power scales quadratically with xbar_size (Confirm with JPS)
    #core_power += num_xbar * ((xbar_size/128.0)**2) * xbar128_pow * time_step_dict[str(xbar_size)] * (16+1)
    ...
    core_power += num_adc * adc_pow * (xbar_size/128.0)**(2.3)
    ...
    core_power += math.sqrt((xbar_size/128.0) * (num_xbar/16.0)) * dataMem128_2_pow * regfile_size
    ...
    tile_power += math.sqrt((xbar_size/128.0)*(num_xbar/16.0)) * edram_pow
    # Less send and receives between tiles
    tile_power += tile_instrnMem_pow / (math.sqrt((xbar_size/128.0)*(num_xbar/16.0)*(num_core/8.0)))
    tile_power += math.sqrt((xbar_size/128.0)*(num_xbar/16.0)) * counter_buff_pow
    tile_power += math.sqrt(num_core/8.0) * edram_bus_pow
    tile_power += math.sqrt((xbar_size/128.0)*(num_xbar/16.0)) * edram_ctrl_pow
    # Less tiles will fan-out to a next layer tile
    # Less send and receives between tiles
    tile_power += receive_buffer_pow
    tile_power += tcu_pow
    tile_power += router_pow
    #return (tile_power * mw * ns)
        ...
        for num_xbar in num_xbar_list:
            ...
            for num_core in num_core_list:
                for regfile_size in regfile_size_list:
                    tile_power = get_tile_power (xbar_size, num_xbar, num_vfu, num_core, regfile_size)
                    num_comp_total = get_num_comp (xbar_size, num_xbar, num_vfu, num_core)
                    time = get_time (xbar_size, num_xbar, num_vfu, num_core)
                    pe = num_comp_total / tile_power / time / (10**9) #inGops/s/Watt
...
print (max(pe_list))
idx = pe_list.index (max(pe_list))
print (name_list[idx])

plt.bar(range(len(pe_list)), pe_list, align='center')
