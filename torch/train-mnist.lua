...
--
local opt = lapp[[
   -m,--maxepoch      (default 10)          maximum epochs for which traiing runs
   -s,--save          (default "logs")      subdirectory to save logs
   -n,--network       (default "")          reload pretrained network
   -m,--model         (default "mlp")       type of model tor train: convnet | mlp | linear
   -f,--full                                use the full dataset
   -p,--plot          (default 1)           plot while training
...
if opt.network == '' then
   ...
   elseif opt.model == 'mlp' then
      ...
      model:add(nn.Linear(1024, 16))
      model:add(nn.Tanh())
      model:add(nn.Linear(16,#classes))
      model:add(nn.Tanh())
      ------------------------------------------------------------
...
function train(dataset)
   ...
   local filename = paths.concat(opt.save, 'mnist_mlp_1024_1024_10.net')
